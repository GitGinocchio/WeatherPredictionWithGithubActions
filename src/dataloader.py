...
def download_city_data(report : str, city : str) -> tuple[str, str, None | Exception]:
	resource_url = f'https://raw.githubusercontent.com/GitGinocchio/weather-prediction-with-github-actions/data/reports/{report}/{city}.json'
	...
	try:
		...
		if os.path.exists(f'./data/reports/{report}/{city}.json'):
			return (report, city, None)

		os.makedirs(f'./data/reports/{report}', exist_ok=True)

		with open(f'./data/reports/{report}/{city}.json', 'w') as file:
...
def stream_data_on_disk():
	...
	try:
		...
		with ThreadPoolExecutor(max_workers=len(config['sample-cities'])) as executor:
			futures = [
				executor.submit(download_city_data, report, city) 
				for report,cities in tqdm(reports,desc='Preparing weather reports',unit=' report') 
				for city in config['sample-cities']
				if city in cities
			]

			for future in tqdm(as_completed(futures), total=num_reports*len(config['sample-cities']),desc='Downloading weather reports',unit=' city'):
				try:
					report, city, result = future.result(timeout=10)
...
#for i in stream_data_in_memory():
	#pass
